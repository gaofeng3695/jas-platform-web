		...
		if (oid != null && oid != "" ) {
			$.ajax({
				...
				error : function(result) {
					window.showAlert(getLanguageValue("tip"), getLanguageValue("queryError"), 'info');
				}
			});
		} 
	});
...
function save() {
	...
	if(validateResault == false){
		window.showAlert(getLanguageValue("tip"), getLanguageValue("formVailidateFailed"), 'info');
		...
		$.ajax({
		    ...
		    success: function(data){
				if(data.status==1){
					window.showAlert(getLanguageValue("tip"), getLanguageValue("savesuccess"), 'info', function() {
						//重新加载表格数据
					...
					window.showAlert(getLanguageValue("tip"), data.msg, 'error');
					enableButtion("saveButton");
				}
		    }
		 });
