...
function updateFile() {
	var newFileName = $("#filename1").val() + "."+filenameSuf;
	$("#filename1").val(newFileName);
	...
	$('#editfilefrom').form('submit', {
		url: rootPath + 'jasdoc/folder/doccenter/updateDoc.do' + "?token=" + localStorage.getItem("token"),
		onSubmit: function () {
...
$(function () {
	...
	$.getJSON(rootPath + 'jasdoc/folder/doccenter/getFileInfoById.do', {
		'docId': eventid
	}, function (obj) {
		$("#eventid").val(obj.eventid);
		$("#filename").val(obj.filename);
		var file = obj.filename.split(".");
		$("#filename1").val(file[0]);
		filenameSuf = file[1];
