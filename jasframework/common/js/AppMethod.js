...
function geoAreaQuery(strGeometryJson)
{
	...
	{		
		...
		{		
			...
			top.tab.openTab("03010102","空间搜索","../../gis/geosearch/geosearch.htm");
		}
		else
		{			
			top.tab.openTab("03010102","空间搜索","../../gis/geosearch/geosearch.htm");				
...
/**
 * 
 * 功能描述：根据客户端ip获取客户端所在地（伊拉克、迪拜）
 *
 * @returns 客户端所在地（伊拉克、迪拜）
 */
function getClientArea(){
	var clientIp = getParamter('clientIp');
	//alert("登录客户端ip为："+ clientIp);
	clientIp = "192.168.10.100";
	//alert("登录客户端ip为："+ clientIp);
	var dbIpScope = ipScopeOfArea.DB;
	var ylkIpScope = ipScopeOfArea.YLK;
	
	var res1 = [];
	var addr1 = clientIp.split(".");
	res1.push(addr1[0]);
	res1.push(addr1[1]);
	res1.push(addr1[2]);
	var clientIpScope = res1.join(".");
	if(dbIpScope.indexOf(clientIpScope)>-1){
		return "db";
	}
	if(ylkIpScope.indexOf(clientIpScope)>-1){
		return "ylk";
	}
	return clientIp;
}
...
function getMapTitleUrl(){
	var clientArea = getClientArea();
	if(clientArea=="db"){
		//alert("登录客户端区域为：迪拜");
		return dbMap;
	}else if(clientArea=="ylk"){
		//alert("登录客户端区域为：伊拉克");
		return ylkMap;
	}
}