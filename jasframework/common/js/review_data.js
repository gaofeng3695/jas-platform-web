...
function lastData(){
	...
	//var ht = $("#contentArea").height();
	//$("#contentArea").height(ht -50);
	console.log($("#contentArea").height());
	//$("#contentArea").height($(".tabs-panels").height()-120);
...
//		idArray = [];
	...
	if(!isNull(toPassidArray)){
		...
		$.messager.confirm('审核', '您确定审核通过吗？\n\t',function(r){
			if (r){
				$.ajax({
				   url: rootPath+"/consDcsCommon/passData.do?className="+className,
				   ...
				   success: function(data){
						if(data.status==1){
							...
							top.showAlert("错误","审核失败","error");
						}
				   },
				   error : function(data) {
						top.showAlert('错误', '审核出错', 'info');
					}
				});
			}
		});	
...
//		idArray = [];
	...
	if(!isNull(toPassidArray)){
		...
		$.messager.confirm('审核', '您确定审核退回吗？\n\t',function(r){
			if (r){
				$.ajax({
				   url: rootPath+"/consDcsCommon/notPassData.do?className="+className,
				   contentType: 'application/json;charset=utf-8',
				   data: JSON.stringify({"idArray" : toPassidArray,"advice":advice}),
				   type: "POST",
				   dataType:"json",
