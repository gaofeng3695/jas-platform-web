$(function() {
	...
	$("#chartTable").datagrid({
		url:rootPath + "/jasframework/statisticalchart/querychartByuserId.do",
		rownumbers:true,
		pagination:true,
		nowrap:true,
		toolbar:"#tb",   
...
function updateChart() {
	...
	if (rows.length == 1) {
		var row = $('#chartTable').datagrid('getSelected');
		var eventID = row.eventid;
		top.getDlg("addAndUpdateChart.htm?mychart="+"yes"+"&eventid=" + eventID, "saveiframe", "修改",
				700, 650);
	} else {
		$.messager.alert('提示', '请选中一条记录', 'info');
	}
...
function addChart(){
	top.getDlg("addAndUpdateChart.htm?mychart="+"yes" , "saveiframe", "新增",
			700, 650);
}
