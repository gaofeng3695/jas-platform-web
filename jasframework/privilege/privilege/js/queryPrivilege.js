...
$(document).ready(function () {
	...
	function my_resize(panelWidth) {
		...
		$('#left').panel('resize', {
			height: clientHeight,
			width: div_left_width
		});
		$('#right').panel('resize', {
			height: clientHeight,
			width: clientWidth - panelWidth
		});
...
function zTreeOnClick(event, treeId, treeNode, clickFlag) {
	...
	if (treeNode.isParent) {
		$("#P-PRI-0045").linkbutton("disable");
	} else {
		$("#P-PRI-0045").linkbutton("enable");
	}
}
/**
 * 异步加载成功之后
...
function privilegeConfig() {
	...
	var treeObj = $.fn.zTree.getZTreeObj("tt");
	var nodes = treeObj.getSelectedNodes();
	// url = "/jasmvvm/pages/module-template/base-template-new/base-template-new-easyui.html?appId=" + appId + "&currentNodeId=" + nodes[0].id+'&pageCode=confiPrivilege';
	// top.getDlg(url, "configPrivilege", "功能权限配置", 700, 620);
	top.jasTools.dialog.show({
		width: '60%',
		height: '80%',
		title: '功能配置权限',
		src: jasTools.base.rootPath +
			'/jasmvvm/pages/module-template/base-template-new/base-template-new.html?appId=' + appId + '&currentNodeId=' + nodes[0].id + '&pageCode=confiPrivilege',
