			...
			onclose:function(){
				if(currentPageLayout == 1){
					if(clickStatis || click3dStatis){
						innerLayout.sizePane('west',innerLayout.state.west.size);
						if(click3dStatis){
...
function createInnerLayout ( currentPageLayout ){
	...
	console.log($Container.data("layoutContainer"))
...
//					console.log("9999");
	...
	var $Containers = $('body >.ui-layout-center> .ui-layout-west');
...
//	}
	...
	if(!clickStatis){
		...
		if(currentPageLayout ==1){
			...
			innerLayout.sizePane('west',  $('#outerCenter').width());
			if(isHasOpen3d){
		...
		clickStatis =false;
		if(currentPageLayout ==1){
			innerLayout.sizePane('west',$('#outerCenter').width()/2);
