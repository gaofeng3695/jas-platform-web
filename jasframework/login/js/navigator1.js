			...
			if(data.status==1){
				...
				for(var i=0;i<rows.length;i++){
					...
					if(childrenList.length>0){
						str+='<span class="title" data-value='+rows[i].OID+'>'+rows[i].NAME+'</span>'+
			  			     '<div class="content">';
						for(var j=0;j<childrenList.length;j++){
							str += '<button class="btn" data-value='+childrenList[j].eventid+'>'+childrenList[j].name+'</button>';
						}
						str +='</div>';
					}
				}
	...
	$.ajax({
	   ...
	   success: function(data){
			if(data.status==1){
				...
				var rows = data.data;
				var hasMenuObj = {};
				$("#section").find(".title").each(function(){
					var hasMenu = "";
					$(this).next().find(".btn").each(function(){
						hasMenu += $(this).data("value")+",";
					});
					hasMenuObj[$(this).data("value")] = hasMenu;
				});
				...
				for(var i=0;i<rows.length;i++){
					...
					if(childrenList.length>0){
						...
						console.log(hasMenuStr)
						str+= '<span class="title" data-value='+rows[i].OID+'>'+rows[i].NAME+'</span>'+
			  			      '<div class="content">';
						for(var j=0;j<childrenList.length;j++){
							if(hasMenuStr && hasMenuStr.indexOf(childrenList[j].eventid) > -1){
								str +='<span class="menu-position">'+
							  			  '<button class="btn checked" data-value='+childrenList[j].eventid+'>'+childrenList[j].name+'</button>'+
						  			  '</span>';
							}else{
								str +='<span class="menu-position">'+
					  			  '<button class="btn" data-value='+childrenList[j].eventid+'>'+childrenList[j].name+'</button>'+
				  			  '</span>';
							}
				...
				$("#saveButton").click(function(){
					...
					$(".section-cus").find(".title").each(function(index){
						var projectOid = $(this).data("value");
						$(this).next().find(".btn.checked").each(function(index){
...
function createMask() {
    ...
    var domStr = '<div id="shield" class="shield" style=\"' + shieldStyle + '\">'+
				    '<div class="pipeline pipeline-cus">'+
						'<div class="pipeline-head"></div>'+
						'<div class="pipeline-body"></div>'+
						'<div class="pipeline-foot"></div>'+
					'</div>'+
	...
	//点击项目子节点,切换选中状态
	$("#sectionCustom").on("click",".btn",function(){
