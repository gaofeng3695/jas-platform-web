...
var ToolsBar = function(dom,data){
    ...
    var toolbarBtnClick =function (e){
        ...
        switch(target.title){
            ...
            case '放大地图': mapApi.levelUp();break;
            case '缩小地图': mapApi.levelDown();break;
...
function mapApiLoaded(){
    ...
    if(typeof M === "function"){
        ...
        mapApi.subscribe(mEvents.OptionalLayerAddedEvent,grpimsLayerAdded);// 注册监听事件

        mapApi.startup();
        
        $("img[title='图层控制']")[0].click();
        var dataTree = $("#layerListTree",$("#mapLayerListSliderPanel")[0])[0];
        $(dataTree).tree('check',$(dataTree).tree('find','DCS_MAP15').target)
        $(dataTree).tree('check',$(dataTree).tree('find','DCS_MAP02').target)
        $(dataTree).tree('check',$(dataTree).tree('find','DCS_MAP00').target)
        $(dataTree).tree('check',$(dataTree).tree('find','DCS_MAP03').target)
        $(dataTree).tree('check',$(dataTree).tree('find','DCS_MAP01').target)
    }else{
...
var grpimsLayerAdded = function(e){
    ...
    if(e.layer.id === "DCS_MAP13"){
        mapApi.addLayerClickEventListener(e.layer.id,mnLightningStaticClicked);
    }
...
var baseMarkerClicked = function(e) {
	var dataId = e.graphic.attributes.OID;
	console.info(e);
	top.getDlg("../../gas/base/base_marker/view_base_marker.html?oid="+dataId,"viewBaseMarker","详细",800,200,false,true,true);
};

var baseMarkerDetectClicked = function(e) {
	var dataId = e.graphic.attributes.OID;
	console.info(e);
	top.getDlg("../../gas/base/base_detect_marker/view_base_detect_marker.html?oid="+dataId,"viewBaseDetectMarker","详细",800,200,false,true,true);
};
