...
$(function(){
	 var oldnodename ="";
	 $('#i18ntree').tree({		
		url:rootPath + 'jasframework/I18N/geti18ntree.do',
		...
		onClick:function(node){
				...
				rightbuttonobj.menu({
	                 onClick:function(item){
	                	 if( item.name == '002' ){
	                		 ...
	                		 var url =rootPath + "jasframework/I18N/deletei18n.do?t="+new Date().getTime();
	                		 $.ajax({
	           				  url: url,
	           				  cache: false,
			...
			node.id=newnodeid;
			var url = rootPath + "jasframework/I18N/saveandupdatei18n.do?t="+new Date().getTime();
			$.ajax({
				  url: url,
				  cache: false,
...
function i18nbyname(id,text){
	...
	var url =rootPath + "jasframework/I18N/geti18nbyname.do?id="+id+"&text="+text;
	$.ajax({
		  url: url,
		  cache: false,
		  success: function(data){
...
function savei18ninfo(){
	...
	if(i18ninfo==null||i18ninfo==""){
		...
		var url =rootPath + "jasframework/I18N/savei18ninfo.do?t="+new Date().getTime();
		$.ajax({
			  type: "POST",
			  url: url,
			  cache: false,
